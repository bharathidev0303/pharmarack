...
class CustomExpandableTile extends StatefulWidget {
  ...
  final Widget Function(bool isExpanded, Animation<double> iconTurns,
      Animation<double> heightFactor) header;
  ...
  State<StatefulWidget> createState() {
    return CustomTileTileState();
  }
}

class CustomTileTileState extends State<CustomExpandableTile>
  ...
  void initState() {
    ...
    _heightFactor = _controller.drive(CustomExpandableTile._easeInTween);
    _iconTurns = _controller.drive((widget.animatedWidgetTurnTween ??
            CustomExpandableTile._halfTween)
        .chain(
            widget.animatedWidgetTween ?? CustomExpandableTile._easeInTween));

    _borderColor = _controller.drive(_borderColorTween.chain(
        widget.borderAnimationTween ?? CustomExpandableTile._easeOutTween));
    _borderColorTween.end = widget.borderColorEnd;

    _headerColor = _controller.drive(_headerColorTween.chain(
        widget.headerAnimationTween ?? CustomExpandableTile._easeInTween));
  ...
  Widget _getHeader() {
    if (!_isExpanded) {
      return widget.header(_isExpanded, _iconTurns, _heightFactor);
    } else {
      return widget.headerExpanded ??
          widget.header(_isExpanded, _iconTurns, _heightFactor);
    }
  }
